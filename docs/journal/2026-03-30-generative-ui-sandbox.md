...
**Cloudflare auto-wiring:** Originally planned to check for sandbox + browser rendering bindings directly in `prepareRunner()` and auto-wire. Couldn't import `@positronic/surface` or `@cloudflare/sandbox` in `brain-runner-do.ts` — React dependencies break vitest/workerd, and even dynamic imports get resolved statically by Vite. Solution: `setPlatformPlugins()` setter (same pattern as `setBrainRunner`, `setManifest`). The user's `index.ts` calls it at module init. Brain-level `withPlugin(surface.setup(...))` overrides by plugin name.

**Privacy boundary:** Surface never sees real data. It generates pages with fake data (for previews), and core does a local regex replacement of `window.__POSITRONIC_DATA__` with real data after receiving the HTML. This matters for the hosted service model — user data stays on the caller's infrastructure. The regex matches the full `<script>` tag and escapes `<` in JSON to prevent `</script>` injection.
