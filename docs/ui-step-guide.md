...
Forms generated by `.ui()` steps include automatic CSRF token protection. A unique token is embedded as a hidden field (`__positronic_token`) in the form HTML and stored alongside the webhook registration. When the form is submitted, the server validates that the submitted token matches the stored token. Submissions with missing or invalid tokens are rejected.
...
2. Add `<input type="hidden" name="__positronic_token" value="...">` to your form HTML
