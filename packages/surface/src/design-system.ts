/**
 * Design system document injected into the LLM system prompt during page generation.
 * The importPath parameter controls where the LLM imports components from —
 * this depends on how the sandbox filesystem is configured.
 */
export function buildDesignSystemDoc(importPath: string) {
  return DESIGN_SYSTEM_TEMPLATE.replaceAll('{{IMPORT_PATH}}', importPath);
}

/**
 * Default design system doc with @positronic/surface as import path.
 * Use buildDesignSystemDoc() if you need a custom import path for the sandbox.
 */
export const DESIGN_SYSTEM_DOC = buildDesignSystemDoc('@positronic/surface');

const DESIGN_SYSTEM_TEMPLATE = `# Surface Design System
...
All components are available from \`{{IMPORT_PATH}}\`:

\`\`\`tsx
import { Card, CardHeader, CardTitle, CardContent, Button, Badge } from '{{IMPORT_PATH}}';
\`\`\`
