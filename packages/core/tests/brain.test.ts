...
describe('brain structure', () => {
  ...
  describe('step timeout behavior', () => {
    ...
    it('should timeout a step and retry when step takes too long', async () => {
      let callCount = 0;
      const testBrain = brain('Timeout Brain').step('Slow step', async () => {
        callCount++;
        if (callCount === 1) {
      ...
      // Advance time past the 25s timeout for first attempt
      await jest.advanceTimersByTimeAsync(26_000);

      // Advance time for second attempt (which succeeds immediately)
      await jest.advanceTimersByTimeAsync(100);

      await runPromise;

      // Verify step was called twice (timeout + retry)
      expect(callCount).toBe(2);

      // Verify STEP_RETRY event was emitted with timeout error
      const retryEvent = events.find((e) => e.type === BRAIN_EVENTS.STEP_RETRY);
      expect(retryEvent).toBeDefined();
      expect(retryEvent).toEqual(
        expect.objectContaining({
      ...
      // Verify brain completed successfully after retry
      const completeEvent = events.find(
        (e) => e.type === BRAIN_EVENTS.COMPLETE
      );
      expect(completeEvent).toBeDefined();
    ...
    it('should emit ERROR event when step times out twice', async () => {
      let callCount = 0;
      const testBrain = brain('Always Timeout Brain').step(
        'Always slow',
        async () => {
          callCount++;
          // Always hang - will always timeout
          await new Promise((resolve) => setTimeout(resolve, 60_000));
          return { success: true };
        }
      );
      ...
      // Advance time past the 25s timeout for second attempt (retry)
      await jest.advanceTimersByTimeAsync(26_000);

      await runPromise;

      // Verify step was called twice (initial + retry)
      expect(callCount).toBe(2);
      ...
      // Verify STEP_RETRY event was emitted for first timeout
      const retryEvent = events.find((e) => e.type === BRAIN_EVENTS.STEP_RETRY);
      expect(retryEvent).toBeDefined();

      // Verify ERROR event was emitted after second timeout
      const errorEvent = events.find((e) => e.type === BRAIN_EVENTS.ERROR);
      expect(errorEvent).toBeDefined();
      expect(errorEvent).toEqual(
        expect.objectContaining({
    ...
    it('should not timeout steps that complete within the limit', async () => {
      const testBrain = brain('Fast Brain').step('Fast step', async () => {
        // Short delay that won't trigger timeout
      ...
      // Advance time just enough for the step to complete
      await jest.advanceTimersByTimeAsync(200);

      await runPromise;

      // Verify no STEP_RETRY event (no timeout occurred)
      const retryEvent = events.find((e) => e.type === BRAIN_EVENTS.STEP_RETRY);
      expect(retryEvent).toBeUndefined();
