...
**Page forms** (user submits a form on a page generated by your brain) are handled automatically by `.page()` with `outputSchema`. No webhook code needed — see "Custom HTML Pages" below and `/docs/brain-dsl-guide.md`.
...
Instead of writing HTML, use `message` + `inputData` to have the AI generate the page with React and shadcn components:

```typescript
.page('Review Emails', ({ state: { emails } }) => ({
  message: 'Show an email digest with checkboxes to deselect threads',
  inputData: { emails },
  outputSchema: z.object({ deselectedIds: z.array(z.string()) }),
}))
```

- `inputData` wraps state fields — the framework infers the schema from the runtime data and stamps array counts automatically. Field names guide the AI's fake data generation and UI design.
