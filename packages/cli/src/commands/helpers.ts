 ...
 */
export function generateSecretTypes(projectRootPath: string): string {
...
`;
  ...
  const secretNames = Object.keys(parsed).filter(isValidJSIdentifier);

  // Generate interface properties
  const properties = secretNames
    .map((name) => `    ${name}: string | undefined;`)
    .join('\n');
 ...
 */
export function generateAndWriteSecretTypes(projectRootPath: string): string {
  const typesFilePath = path.join(projectRootPath, 'secrets.d.ts');
  const typeDefinitions = generateSecretTypes(projectRootPath);
