...
export class PromptCommand {
    ...
    show(argv: ArgumentsCamelCase<{ promptName: string }>): void {
        console.log(`Showing prompt details for: ${argv.promptName} including usage statistics`);
    ...
    new(argv: ArgumentsCamelCase<{ promptName: string }>): void {
        ...
        if (!this.projectRootPath) {
            console.error("Internal Error: Project root path not available for prompt new command.");
            process.exit(1);
        }

        console.log(`Creating new prompt in project ${this.projectRootPath}: ${argv.promptName}`);
