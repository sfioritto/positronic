...
// NOTE: Must be defined BEFORE the generic /webhooks/:slug route
app.post('/webhooks/ui-form', async (context: Context) => {
  ...
  try {
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (key.endsWith('[]')) {
        ...
        (response[baseKey] as unknown[]).push(value);
      } else if (response[key] !== undefined) {
        // Convert to array if same key appears multiple times
        if (!Array.isArray(response[key])) {
          response[key] = [response[key]];
        }
        (response[key] as unknown[]).push(value);
      } else {
        response[key] = value;
      }
    }

    // Find the brain waiting for this webhook
    const monitorId = context.env.MONITOR_DO.idFromName('singleton');
    const monitorStub = context.env.MONITOR_DO.get(monitorId);
    const brainRunId = await monitorStub.findWaitingBrain('ui-form', identifier);
    ...
    // No brain waiting for this form submission
    return context.json({
      received: true,
      action: 'not_found',
      identifier,
